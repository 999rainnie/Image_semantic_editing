...
class StableDiffusionFreeGuidancePipeline(StableDiffusionAttendAndExcitePipeline):    
    ...
    def register_attention_control(self):
        ...
        for name in self.unet.attn_processors.keys():
            ...
            cross_att_count += 1
            attn_procs[name] = CustomAttnProcessor(attnstore=self.attention_controller, place_in_unet=place_in_unet)

        self.unet.set_attn_processor(attn_procs)
        self.attention_controller.num_att_layers = cross_att_count
    ...
    def sample(self, latents, scheduler, t, feature_layer, guidance_scale, cond_prompt_embeds, prompt_embeds, cross_attention_kwargs, hook=None, pred_type='edit', set_store=True, do_classifier_free_guidance=True):   
        ...
        # print(latent_model_input.shape, cond_prompt_embeds.shape)
    ...
    def __call__(
        ...
        all_latents: Optional[List[torch.FloatTensor]] = None,
                ...
                if is_guidance:
                    with torch.enable_grad():
                        for guidance_iter in range(max_guidance_iter_per_step):
                            latents = self.attention_controller.replace_latents(latents, all_latents[index])
                            ...
                            if self.do_self_guidance(i, len(self.scheduler.timesteps), self.scheduler):
                                ## TODO:: give guidance_function - tv loss, semantic feature loss
                                loss = guidance_func(self.attention_controller, indices, ori_feats=ori_feats, edit_feats=edit_feats)
        ...
        edit_image, has_nsfw_concept = self.decode(latents, output_type, device, prompt_embeds) # -> latents
